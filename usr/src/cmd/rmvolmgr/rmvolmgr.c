...
/*
 * These are "private" interfaces for illumos, exported in libc.  But
 * we want to go ahead and make use of them.  Technically, we could
 * rewrite this legacy code to use the public interfaces, and we should
 * do that.  Later.
 */
...
extern int __init_daemon_priv(int flags, uid_t uid, gid_t gid, ...);
